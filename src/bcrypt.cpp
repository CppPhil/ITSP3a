...
boost::optional<std::string> Bcrypt::findHashOfUser(boost::string_ref username)
{
    ...
    while (Record::read(fs, &currentRecord)) {
        if (currentRecord.getUsername() == username) { // it's the same username
            return boost::make_optional(currentRecord.getHash().to_string());
        }
    }

    ITSP3_LOG << "Username \"" << username << '"' << " was never found "
              << "in the binary file.";
    return boost::none; // no hash found for username given
}

bool Bcrypt::isLengthOk(boost::string_ref str) noexcept
{
